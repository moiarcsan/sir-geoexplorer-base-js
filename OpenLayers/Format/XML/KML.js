...
OpenLayers.Format.KML = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    createPlacemarkXML: function(feature) {        
        ...
        // Placemark style
        var placemarkStyle = this.createElementNS(this.kmlns, "Style");
        placemarkStyle = this.createStylekXML(feature.style, placemarkStyle); 
    ...
    createStylekXML: function(style, placemarkStyle){

		/*
		 * Actual mapping example only map
		 *	externalGraphic: 
		 * 	strokeColor
		 *	strokeOpacity
		 *	strokeWidth
    	...
    	if(!!style){
    		styleKXML = "";
    		if(!!style.externalGraphic || !!style.fillColor){
    			var iconStyle = this.createElementNS(this.kmlns, "IconStyle");
    			if(!!style.fillColor && !style.externalGraphic){
	    			var fillKMLColor = this.toKMLColor(style.fillColor, style.fillOpacity);
	    			var color = this.createElementNS(this.kmlns, "color");
	    			color.appendChild(this.createTextNode(fillKMLColor));
					iconStyle.appendChild(color);
	    		};
				var scale = this.createElementNS(this.kmlns, "scale");
				scale.appendChild(this.createTextNode("1"));
	    		...
	    		if(!!style.externalGraphic){
	    			...
	    			href.appendChild(this.createTextNode(style.externalGraphic));
	    			icon.appendChild(href);
	    			iconStyle.appendChild(icon);
	    		}
	    		
    			placemarkStyle.appendChild(iconStyle);
    			var labelStyle = this.createElementNS(this.kmlns, "LabelStyle");
    			var scale = this.createElementNS(this.kmlns, "scale");
    			scale.appendChild(this.createTextNode("1"));
    			labelStyle.appendChild(scale);
    			placemarkStyle.appendChild(labelStyle);
    		}
    		if(!!style.strokeColor){
    			...
    			var lineStyle = this.createElementNS(this.kmlns, "LineStyle");
    			var color = this.createElementNS(this.kmlns, "color");
    			color.appendChild(this.createTextNode(this.toKMLColor(style.strokeColor, style.strokeOpacity)));
    			var width = this.createElementNS(this.kmlns, "width");
    			width.appendChild(this.createTextNode(strokeWidth));
    			lineStyle.appendChild(color);
    			lineStyle.appendChild(width);
    			placemarkStyle.appendChild(lineStyle);
    		}
    	}else{
			placemarkStyle.appendChild(this.createTextNode(this.placemarksStyle));
		}
     ...
     * 
     * Parameters:
     * color - {<String>} Color in hex. For example: '#ffaa00'
     * opacity - {<Float>} Opacity percent <1 and >0 
     * 
     * Returns:
     * {String} kml color 
     */
    toKMLColor: function(color, opacity){
    	var kmlOpacity = 'ff'; //default 100%
    	if(!!opacity){
    		kmlOpacity = (opacity * 255);
    		kmlOpacity = this.decimalToHexString(kmlOpacity);
    	}
    	if(!!color
    			&& color.length == 7
    			&& color.split("#").length == 2
    			&& color.split('#')[1].length == 6){
            var kmlColor = kmlOpacity + color.substring(5) + color.substring(3,5) + color.substring(1,3); 
    		return kmlColor.toString();
    	}else{
    		return 'ff00aaff'; //default color
    	}
    },
    ...
    decimalToHexString: function(number) {
        ...
        var tmp_hex = number.toString(16).toUpperCase();
        if(tmp_hex 
            && tmp_hex.split('.')
            && tmp_hex.split('.').length == 2)
            tmp_hex = tmp_hex.split('.')[0];
